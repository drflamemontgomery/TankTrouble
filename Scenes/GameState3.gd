...
func run():
	...
	#debug_render(maze)
	render_maze(maze)
	var p3_spawn = Vector2(randi()%int(maze.size.x)*32+16, randi()%int(maze.size.y)*32+32)
	#output()
	print_debug(String(maze.size.x) + String(maze.size.y))
	print_debug(String(p1_spawn) + String(p2_spawn) + String(p3_spawn))
	maze.output()
...
func _process(delta):
	if $Level.visible:
		if (!$Level/Player && !$Level/Player2) or (!$Level/Player && !$Level/Player3) or (!$Level/Player2 && !$Level/Player3):
			if $Level/Timer:
...
func _on_Timer_timeout():
	...
	if $Level/Player3:
		$Level/CanvasLayer/Label.show()
		$Level/CanvasLayer/Label.text = "Player3 Wins"
		$Level/Timer.queue_free()
		print_debug("Player3 Wins")
		return
	$Level/CanvasLayer/Label.show()
